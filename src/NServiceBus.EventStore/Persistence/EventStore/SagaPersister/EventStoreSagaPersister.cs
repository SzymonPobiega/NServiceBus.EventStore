...
namespace NServiceBus.Persistence.EventStore.SagaPersister
{
    public class EventStoreSagaPersister : ISagaPersister
    {
        ...
        private void CreateIndicesForSaga(IContainSagaData saga)
        {
            ...
            var propertiesToIndex = UniqueAttribute.GetUniqueProperties(saga);

            foreach (var propertyAndValue in propertiesToIndex)
            {
                var propertyName = propertyAndValue.Key;
                var propertyValue = propertyAndValue.Value;
                try
                {
                    AddUniqueIndex(saga.Id, sagaType, saga.OriginalMessageId, propertyName, propertyValue);
                }
                catch (AggregateException ex)
                {
                    ...
                    {
                        var indexEvent = ReadIndex(sagaType, propertyName, propertyValue);
        ...
        {
            var indexStream = BuildIndexStreamName(sagaType, property, value);
            var slice = connectionManager.GetConnection().ReadStreamEventsForwardAsync(indexStream, 0, 1, true).Result;
