namespace NServiceBus.AcceptanceTests.EndpointTemplates
{
    ...
    public static class ConfigureExtensions
    {
        ...
        static async Task ConfigureTestExecution(TestDependencyType type, EndpointConfiguration config, RunSettings settings, string endpointName)
        {
            ...
            {
                throw new InvalidOperationException($"Acceptance Test project must include a non-namespaced class named '{typeName}' implementing {typeof(IConfigureEndpointTestExecution).Name}. See ConfigureEndpointMsmqTransport for an example.");
            }
