        ...
        public static void Run(ScenarioDescriptor scenarioDescriptor, Func<RunDescriptor, bool> remove)
        {
            ...
            foreach (var rundescriptor in runDescriptors)
            {
                ...
                if (rundescriptor.Settings.TryGet("Transport", out type))
                {
                    ...
                    {
                        throw new InvalidOperationException($"Acceptance Test project must include a non-namespaced class named '{configurerTypeName}' implementing {typeof(IConfigureSupportedScenariosForTestExecution).Name}. See ConfigureScenariosForMsmqTransport for an example.");
                    }
